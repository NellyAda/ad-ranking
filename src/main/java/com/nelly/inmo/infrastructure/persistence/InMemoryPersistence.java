...
public class InMemoryPersistence {
	...
	public InMemoryPersistence() {
		...
		ads.add(new AdVO(4, "FLAT", "Ático céntrico muy luminoso y recién reformado, parece nuevo", Arrays.asList(5),
				300, null, null, null));
		...
		ads.add(new AdVO(7, "GARAGE", "Garaje en el centro de Albacete", Collections.<Integer>emptyList(), 300, null,
				null, null));
	...
	/**
	 * Given a picture id, returns the {@link Optional} containing the picture with
	 * id <b>id</id>
	 * 
	 * @param id - the picture identifier
	 * @return an Optional object containing the {@link PictureVO} object if found,
	 *         or an empty Optional if not
	 */
	public Optional<PictureVO> getPicture(Integer id) {
		return pictures.stream().filter(picture -> id.equals(picture.getId())).findAny();
	}

	/**
	 * Updates the ad with the date and the score
	 * 
	 * @param adVO  - cannot be null
	 * @param date  - the new date
	 * @param score - the new score
	 */
	public void update(AdVO adVO, Date date, Integer score) {
		adVO.setScore(score);
		adVO.setIrrelevantSince(date);
	}
}
